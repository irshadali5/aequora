//! Application composition for the `aequora` command-line client.
...
pub use aequora_devtools as devtools;
pub use aequora_inspect as inspect;

/// Parses process arguments, executes one typed command, renders its result, and returns a stable
/// exit category.
#[must_use]
pub fn run_main() -> ExitCode {
    let raw_arguments = env::args().skip(1).collect::<Vec<_>>();
    let output_hint = output_format_hint(&raw_arguments);
    let parsed = match CliRequest::parse(raw_arguments) {
        Ok(parsed) => parsed,
        Err(error) => {
            if output_hint == OutputFormat::Human {
                eprintln!("aequora: {error}");
            } else {
                let envelope = CliErrorEnvelope::new(
                    ErrorCode::InvalidUsage,
                    RetryClass::Never,
                    "invalid command usage or configuration",
                );
                match render_machine(output_hint, &envelope) {
                    Ok(rendered) => eprintln!("{rendered}"),
                    Err(render_error) => eprintln!("aequora: {render_error}"),
                }
            }
            return ExitCode::from(CliExitCode::UsageOrConfiguration as u8);
        }
    };
    let arguments = parsed.execution_arguments();
    if parsed.global.output == OutputFormat::Human
        && arguments.first().map(String::as_str) == Some("conform")
    {
    ...
    match command(arguments) {
        Ok(output) => {
            if parsed.global.output == OutputFormat::Human {
                ...
                let envelope = MachineEnvelope::new(command_name(&parsed), output);
                match render_machine(parsed.global.output, &envelope) {
                    Ok(rendered) => println!("{rendered}"),
                    Err(error) => {
                        eprintln!("aequora: {error}");
                        return ExitCode::from(CliExitCode::OperationFailed as u8);
                    }
                }
            }
            ExitCode::SUCCESS
        ...
        Err(error) => {
            if parsed.global.output == OutputFormat::Human {
                ...
                let envelope = machine_error(&error);
                match render_machine(parsed.global.output, &envelope) {
                    Ok(rendered) => eprintln!("{rendered}"),
                    Err(render_error) => eprintln!("aequora: {render_error}"),
                }
            }
            ExitCode::from(machine_error(&error).exit_code() as u8)
        }
    }
}

fn output_format_hint(arguments: &[String]) -> OutputFormat {
    arguments
        .windows(2)
        .find_map(|values| {
            (values[0] == "--output")
                .then(|| values[1].parse().ok())
                .flatten()
        })
        .unwrap_or(OutputFormat::Human)
}

fn command_name(request: &CliRequest) -> &'static str {
    match &request.command {
        aequora_cli_core::Command::Help => "help",
        aequora_cli_core::Command::Invoke { name, .. } => name.as_str(),
    }
}

fn machine_error(error: &CliError) -> CliErrorEnvelope {
    match error {
        CliError::Usage(_)
        | CliError::UnknownAdapter(_)
        | CliError::UnknownProfile(_)
        | CliError::InvalidPackageName(_)
        | CliError::MissingParent(_) => CliErrorEnvelope::new(
            ErrorCode::InvalidUsage,
            RetryClass::Never,
            "invalid command usage or configuration",
        ),
        CliError::Compatibility(_) | CliError::ProtocolCompatibility(_) => CliErrorEnvelope::new(
        ...
        _ => CliErrorEnvelope::new(
            ErrorCode::OperationFailed,
            RetryClass::InspectBeforeRetry,
            "operation failed; inspect safe diagnostics before retrying",
        ),
    }
}

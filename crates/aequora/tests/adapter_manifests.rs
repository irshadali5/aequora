...
use aequora_store_sqlite::SQLITE_ADAPTER_MANIFEST;
...
fn built_in_adapters_form_a_verified_database_neutral_production_pair() {
    ...
    assert_eq!(
        AdapterRequirements::PRODUCTION_LOCAL.verify(SQLITE_ADAPTER_MANIFEST),
        Ok(())
    );
    assert_eq!(
    ...
    assert_eq!(
        ProductionAdapterPair::verify(SQLITE_ADAPTER_MANIFEST, POSTGRES_ADAPTER_MANIFEST),
        Ok(ProductionAdapterPair {
            local: SQLITE_ADAPTER_MANIFEST,
            authoritative: POSTGRES_ADAPTER_MANIFEST,
        })
    );
}
